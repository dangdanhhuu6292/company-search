...
/* DVK MBR success replaced with then 2020-02-05 */
...
endpointClient.factory('Endpoint', function() {
  ...
  service.set = function(params) {
    ...
    url = params.site +
          ...
          //'scope=' + oAuthScope + '&' +
          //'state=' + state;
          'response_type=code'; // exactonline
...
profileClient.factory('Profile', ['$http', 'AccessToken', '$rootScope', function($http, AccessToken, $rootScope) {
  ...
  service.find = function(uri) {
    ...
    promise.then(function(response) { // was success
        profile = response;
...
directives.directive('oauth', [
  ...
  definition.link = function postLink(scope, element, attrs) {
    ...
    var initAttributes = function() {
      scope.authorizePath = scope.authorizePath || 'oauth2/auth';
    ...
    var compile = function() {
      $http.get(scope.template, { cache: $templateCache }).then(function(html) { // was success
        element.html(html.data);
        $compile(element.contents())(scope);				
		//var linkFn = $compile(html);
		//var newElem = linkFn(scope);
    ...
    var initProfile = function(scope) {
      ...
      if (token && token.access_token && scope.profileUri) {
        Profile.find(scope.profileUri).then(function(response) { // was sucees
